...
__global__ void kernel_SetIC(DAT *Pl, DAT *Pg, DAT *Pc, DAT *Sl,
                             ...
                             DAT *phi, DAT *rhog,
                             DAT *rsd_l, DAT *rsd_g,
                             const DAT K0, const DAT rhol, const DAT rhog0, const DAT vg_a, const DAT vg_m, const DAT vg_n,
...
//        for(int j = 0; j < ny; j++)
//            Kx[i+j*(nx+1)] = 0.01*K0 * (0.2 + rand()/(double)RAND_MAX);
//    cudaMemcpy(dev_Kx,  Kx,  sizeof(DAT) * (nx+1)*ny, cudaMemcpyHostToDevice);
//    for(int i = 0; i < nx; i++)
//        for(int j = 0; j < ny+1; j++)
//            Ky[i+j*nx] = K0 * (0.2 + rand()/(double)RAND_MAX);
...
__global__ void kernel_SetIC(DAT *Pl, DAT *Pg, DAT *Pc, DAT *Sl,
                             ...
                             DAT *rsd_l, DAT *rsd_g,
                             const DAT K0,
                             const DAT rhol, const DAT rhog0,
                                 ...
                                 const DAT dx, const DAT dy)
{
    ...
    if(i > 0 && i < nx && j >= 0 && j <= ny-1){ // Internal fluxes
        int ind = i+j*(nx+1);
        DAT rhog_face = 0.5*(rhog[i-1+j*nx] + rhog[i-1+j*nx]);
        qlx[ind] = -rhol     *Kx[ind]/mul*Krlx[ind]*((Pl[i+j*nx]-Pl[i-1+j*nx])/dx);
        qgx[ind] = -rhog_face*Kx[ind]/mug*Krgx[ind]*((Pg[i+j*nx]-Pg[i-1+j*nx])/dx);
    }

    if(i >= 0 && i <= nx-1 && j > 0 && j < ny){ // Internal fluxes
        int ind = i+j*nx;
        DAT rhog_face = 0.5*(rhog[i+j*nx] + rhog[i+(j-1)*nx]);
        qly[ind] = -rhol     *Ky[ind]/mul*Krly[ind]*((Pl[i+j*nx]-Pl[i+(j-1)*nx])/dy + 0*rhol*g);
        qgy[ind] = -rhog_face*Ky[ind]/mug*Krgy[ind]*((Pg[i+j*nx]-Pg[i+(j-1)*nx])/dy + 0*rhog_face*g);
    }
    ...
    if(i >= 0 && i <= nx-1 && j == 0){
        ...
        qly[i+j*nx] = FLUX;
        //qly[i+j*nx] = -rhol*1e-18/mul/*Krly[i+j*nx]*/*((Pl[i+j*nx]-10e6)/dy + 0*rhol*g);
        //printf("q = %e\n", qly[i+j*nx]);
    }
                                   ...
                                   const int nx, const int ny)
{
    ...
    if(i >= 0 && i < nx && j >= 0 && j < ny){
        ...
        // For old scheme where it works once per time STEP!
//        // Explicit update
        //phi[ind] = 0.16 + c_phi*0.16*(Pf - Pf_old);
...
void Problem::SaveVTK_GPU(std::string path)
{
    ...
    cudaMemcpy(rhog, dev_rhog, sizeof(DAT) * nx*ny, cudaMemcpyDeviceToHost);
